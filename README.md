Prebuilt [ImageMagick](https://github.com/imagemagick/imagemagick) for Debian Bookworm (actually Ubuntu 22.04 but should be runnable on Debian) with minimal runtime dependencies.

Deps used:

| Library | Repository |
| --- | --- |
| zlib | [repo](https://github.com/madler/zlib) |
| libdeflate | [repo](https://github.com/ebiggers/libdeflate) |
| libjpeg-turbo | [repo](https://github.com/libjpeg-turbo/libjpeg-turbo) |
| libpng | [repo](https://github.com/glennrp/libpng) |
| bzip2 | [repo](https://github.com/libarchive/bzip2) |
| zstd | [repo](https://github.com/facebook/zstd) |
| openjpeg | [repo](https://github.com/uclouvain/openjpeg) |
| lcms2 | [repo](https://github.com/mm2/Little-CMS) |
| xz | [repo](https://github.com/tukaani-project/xz) |
| libxml2 | [repo](https://gitlab.gnome.org/GNOME/libxml2) |
| libzip | [repo](https://github.com/nih-at/libzip) |
| pcre2 | [repo](https://github.com/PCRE2Project/pcre2) |
| libffi | [repo](https://github.com/libffi/libffi) |
| glib | [repo](https://github.com/GNOME/glib) |
| gvdb | [repo](https://gitlab.gnome.org/GNOME/gvdb) |
| liblqr | [repo](https://github.com/carlobaldassi/liblqr) |
| fribidi | [repo](https://github.com/fribidi/fribidi) |
| libraqm | [repo](https://github.com/HOST-Oman/libraqm) |
| imath | [repo](https://github.com/AcademySoftwareFoundation/Imath) |
| openexr | [repo](https://github.com/AcademySoftwareFoundation/openexr) |
| libde265 | [repo](https://github.com/strukturag/libde265) |
| libheif | [repo](https://github.com/strukturag/libheif) |
| libraw | [repo](https://github.com/LibRaw/LibRaw) |
| freetype | [repo](https://github.com/freetype/freetype) |
| harfbuzz | [repo](https://github.com/harfbuzz/harfbuzz) |
| libwebp | [repo](https://github.com/webmproject/libwebp) |
| libtiff | [repo](https://gitlab.com/libtiff/libtiff) |
| fontconfig | [repo](https://gitlab.freedesktop.org/fontconfig/fontconfig) |
| meson | [repo](https://github.com/mesonbuild/meson.git) |

Disclaimer: This repo is almost purely vibecoded with copilot.
